...
if ask_for_confirmation "Do you want to set up Solr Replication?"; then
  ...
  read -p "Enter your Solr host (e.g., 10.0.0.1:8983/solr): " solr_host
  sudo docker exec "$solr_container_id" sed -i "s#<str name="allowUrls">\${solr.allowUrls:}</str>#<str name="allowUrls">\${solr.allowUrls:$solr_host}</str>#g" "$bitnami_solr/solr.xml"
