...
| solr-replication | solr | 8983 | bitnami/solr:9.8.1 | Using bitnami instead of default solr because of the env options. We're mounting the `/bitnami` directory to either the solr_follower_data docker volume, or a local directory through the environment variable `SOLR_FOLLOWER_VOLUME`, which allows us to manipulate the core configuration. We pass SOLR_CORES with a list of all our cores. We pass `SOLR_OPTS` containing memory options. We're using `SOLR_ADMIN_USERNAME` and `*_PASSWORD` to use authentication. |
