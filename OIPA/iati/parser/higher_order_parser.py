...
def provider_org(self, parent_model, provider_model, fk_name):
    ...
    def func(element):
        ...
        if provider_activity_id and not provider_activity:
            self.append_error('FieldValidationError',"transaction/provider-org", "provider-activity-id", "Must be an existing IATI activity", element.sourceline)
...
def receiver_org(self, parent_model, receiver_model, fk_name):
    ...
    def func(element):
        ...
        if receiver_activity_id and not receiver_activity:
            self.append_error('FieldValidationError',"transaction/receiver-org", "receiver-activity-id", "Must be an existing IATI activity", element.sourceline)
