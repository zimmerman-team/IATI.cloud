...
class Parse(Parse_2_01):
    ...
    def iati_organisations__iati_organisation__name(self, element):
        ...
        # check if names exist, if so remove and add all new ones + set
        # primary_name
    ...
    def iati_organisations__iati_organisation__reporting_org(self, element):
        ...
        super(
            Parse, self
        ).iati_organisations__iati_organisation__reporting_org(element)

        # check if name element exists, If not and no org and org has no name,
        # add this as org name + primary name

        organisation_reporting_organisation = self.get_model(
        ...
        if element.text:
            ...
            if organisation.name.narratives.count() == 0:
                organisation.primary_name = element.text
                self.add_narrative(element, organisation.name,
