...
class OrganisationIndexing(BaseIndexing):
    ...
    def related_budget(
        ...
        if related_budget_all:
            ...
            for related_budget in related_budget_all:
                self.add_value_list(
                    prefix + '_period_start', value_string(
                        related_budget.period_start
                    )
                )
                self.add_value_list(
                    prefix + '_period_end', value_string(
                        related_budget.period_end
                    )
                )
                self.add_value_list(
                    prefix + '_value', decimal_string(
                        related_budget.value
                    )
                )
                self.add_value_list(
                    prefix + '_value_currency', related_budget.currency_id
                )
                self.add_value_list(
                    prefix + '_value_date', value_string(
                        related_budget.value_date
                    )
                )
    ...
    def name(self):
        ...
        if name:
            ...
            self.indexing['organisation_name_narrative_lang'], 
            self.indexing['organisation_name_narrative'] =
    ...
    def organisation(self):
        ...
        self.add_field(
            'organisation_reported_in_iati', bool_string(
                organisation.reported_in_iati
            )
        )
        self.add_field(
            'organisation_published', bool_string(
                organisation.published
            )
        )
        self.add_field(
            'organisation_last_updated_datetime', value_string(
        ...
        self.add_field(
            'organisation_default_lang_code', organisation.default_lang_id
        )
