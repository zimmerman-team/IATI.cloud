...
class BaseIndexing(serializers.Serializer):
        ...
        if field:
            ...
            if narratives_all:
                ...
                for narrative in narratives_all:
                    value = JSONRenderer().render(
                        NarrativeSerializer(narrative).data
                    ).decode() if is_json_string
    ...
    def related_narrative(
        self, related, narrative_key, narrative_text_key, narrative_lang_key
    ):
        if related:
            for narrative in related.narratives.all():
                ...
                if narrative.language:
                    self.add_value_list(
                        narrative_lang_key, narrative.language.code
                    )
