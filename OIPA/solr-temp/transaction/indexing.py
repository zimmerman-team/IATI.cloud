...
class TransactionIndexing(BaseIndexing):
    ...
    def transaction(self):
        ...
        self.add_field(
            'transaction_provider_org_type', get_child_attr(
                transaction, 'provider_organisation.type_id'
            )
        )
        self.add_field(
            'transaction_provider_org_ref', get_child_attr(
                transaction, 'provider_organisation.ref'
            )
        )

        provider_organisation = getattr(
            transaction, 'provider_organisation', None
        )
        ...
        self.add_field(
            'transaction_receiver_org_type', get_child_attr(
                transaction, 'receiver_organisation.type_id'
            )
        )
        self.add_field(
            'transaction_receiver_org_ref', get_child_attr(
        ...
        receiver_organisation = getattr(
            transaction, 'receiver_organisation', None
        )
        ...
        self.add_field(
            'transaction_finance_type_code', transaction.finance_type_id
        )
        ...
        self.add_field(
            'transaction_tied_status_code', transaction.tied_status_id
        )

        self.add_field(
            'humanitarian', bool_string(
                get_child_attr(
                    transaction, 'activity.humanitarian'
                )
            )
        )
        ...
        for activity_sector in transaction.activity.activitysector_set.all():
            ...
            self.add_value_list(
                'sector_vocabulary', activity_sector.vocabulary_id
            )
            self.add_value_list(
                'sector_vocabulary_uri', activity_sector.vocabulary_uri
            )
            self.add_value_list(
                'sector_code', activity_sector.sector_id
            )
            self.add_value_list(
                'sector_percentage', decimal_string(
