...
class ActivityAggregationSerializer(BaseSerializer):
    ...
    def apply_group_filters(self, queryset, request, groupList):
        ...
        # separate_aggregations = []

        # for grouping in groupList:

        #     if not self._aggregations.get('extra_filter'):
    ...
    def apply_annotations(self, queryset, groupList, aggregationList):
        ...
        # aggregations that can be performed in the same query
        for aggregation in same_query_aggregations:
            annotation = self._aggregations.get(aggregation, None)
            first_queryset = first_queryset.annotate(**annotation["annotate"])

        first_result = queryset.values(*groupList)
        ...
        for aggregation in separate_aggregations:
            ...
            annotation = self._aggregations.get(aggregation, None)
            filter = annotation.get("extra_filter", None)
            field = annotation.get("field", None)
            final_queryset = queryset.annotate(**annotation["annotate"])
            
            print(first_result)

            # join results in results object (assumes)
            for i, (result, newresult) in enumerate(zip(first_result, queryset.values(*groupList))):
                print(result)
                print(newresult)
                result[field] = newresult[field]
