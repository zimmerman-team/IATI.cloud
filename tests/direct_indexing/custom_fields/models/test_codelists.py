...
def test_codelist(mocker, tmp_path, requests_mock, fixture_codelists):
    ...
    with open(tmp_path / 'codelists.json', 'w') as file:
        json.dump(fixture_codelists, file)
    # mock settings.CODELISTS_JSON to tmp_path/codelists.json
    mocker.patch('direct_indexing.custom_fields.models.codelists.settings.CODELISTS_JSON', tmp_path / 'codelists.json')
    ...
    (tmp_path / 'codelists.json').unlink()
    ...
    assert (tmp_path / 'codelists.json').exists()
